...
import { IEggContainer } from "../types";
...
const EggContainer: React.FC<IEggContainer> = ({ information }) => {
  ...
  // 이 이름은 예외처리
  if (
    poketmon[changeNum - 1].name === "Articuno" ||
    poketmon[changeNum - 1].name === "Zapdos" ||
    poketmon[changeNum - 1].name === "Moltres" ||
    poketmon[changeNum - 1].name === "Mewtwo" ||
    poketmon[changeNum - 1].name === "Mew"
  ) {
  ...
  if (information.name === "Egg")
    hatchPokemon = (
      poketmon[changeNum - 1].prev_evolution
        ? poketmon.filter(
            (item) =>
              item.num === poketmon[changeNum - 1].prev_evolution![0].num
          )
        : [poketmon[changeNum - 1]]
    )[0];
  else hatchPokemon = poketmon[changeNum - 1]; // 특별알은 진화된  아이들도 모두 나오게 해준다.

  //예외처리 (nidran (female) , nidoran (male)은 안찾아진다. ,)
  if (hatchPokemon.name.includes("female")) smallName = "nidoran_f";
  else if (hatchPokemon.name.includes("male")) smallName = "nidoran_m";
  else if (hatchPokemon.name === "Farfetch'd") smallName = "farfetchd";
  else if (hatchPokemon.name.includes("Mime")) smallName = "mr._mime";
  else smallName = hatchPokemon.name.toLowerCase();

  hatchPokemon = {
    ...hatchPokemon,
    myId: myPokemons.length + 1,
    cp: Math.floor(Math.random() * (900 - 100)) + 100,
    health: 100,
    commonUrl: commonUrl + smallName + ".gif",
    commonBackUrl: commonBackUrl + smallName + ".gif",
    shinyUrl: shinyUrl + smallName + ".gif",
    shinyBackUrl: shinyBackUrl + smallName + ".gif",
    color: 0,
  };

  if (
    smallName.includes("nidoran") ||
    smallName.includes("mime") ||
    smallName.includes("fetchd")
  ) {
  ...
  return (
    <EggWrapper>
      <img src={information.img} alt="egg" />
      <img src={hatchPokemon.commonUrl} alt="egg" />
    </EggWrapper>
  );
};
