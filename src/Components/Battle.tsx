...
const Battle: React.FC<IBattle> = ({
  ...
  const attack = () => {
    ...
    if (
      ...
      // 내가 더 높으면  추가 데미지를 준다.  // 같으면  서로 10씩 만 뺏어간다.
  ...
  const InitWindowEnter = (item: HTMLElement) => {
    // 초기하면 리스트에서  목록을고르고  엔터나 스페이스바 누를경우
    ...
    if (item.className.includes("attack")) {
      ...
      if (myBattlePokemons.length !== 1) {
        ...
        menu.current.innerHTML =
          "<li>There is  no Pokemon to be exchanged</li> <li>뒤로가기</li>";
    } else if (menu.current && item.innerHTML === "뒤로가기") {
      menu.current.innerHTML = "";
      setIndex(0);
  ...
  const myPokeomnsSetting = () => {
    // 배틀이 끝났을때  배틀포켓몬들의 체력을  mypokemon들 저장소 에 새로 반영
