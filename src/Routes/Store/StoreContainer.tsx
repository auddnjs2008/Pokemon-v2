...
const StoreContainer = () => {
  ...
  const [myMoney, setMyMoney] = useState(
    JSON.parse(localStorage.getItem("myBag")!).money
  );
  ...
  const handleBuyBtn = () => {
    ...
    if (itemName === "pokeballs" || itemName === "Incenses") {
      // 묶음 아이템을 살 경우
    ...
    if (Badges.length === 0)
      // 산 아이템이 뱃지가 아닐경우
      itemes[itemName] =
        itemes[itemName] !== undefined ? itemes[itemName] + number : number;
    // 산 아이템이 뱃지일 경우
    else {
      if (
        itemes["Badges"] === undefined ||
        !itemes["Badges"].includes(Badges[0])
      )
        itemes["Badges"] =
          itemes["Badges"] !== undefined
            ? [...itemes["Badges"], ...Badges]
            : Badges;
      else {
        // 알람메시지를 넣어준다.
    ...
    if (
      itemes["Badges"] &&
      BadgesNumber !== itemes["Badges"].length &&
      (itemes["Badges"].length === 10 || itemes["Badges"].length === 23)
    ) {
      const newPokemonName =
        megaPokemon[Math.floor(Math.random() * (megaPokemon.length - 1)) + 1];

      let newfilterPokemon = pokemons.pokemon.filter(
        (item) => item.name.toLowerCase() === newPokemonName
      );
      let newPokemon = {
        ...newfilterPokemon[0],
        myId: myPokemons.length + 1,
        cp: Math.floor(Math.random() * (2000 - 1500)) + 1500,
        health: 100,
        commonUrl: urlSearch.commonUrl(newPokemonName),
        commonBackUrl: urlSearch.commonBackUrl(newPokemonName),
        shinyUrl: urlSearch.shinyUrl(newPokemonName),
        shinyBackUrl: urlSearch.shinyBackUrl(newPokemonName),
        color: 0,
        specialUrl: (
          urlSearch.megaUrl(newPokemonName) as {
        ...
        specialBackUrl: (
          urlSearch.megaBackUrl(newPokemonName) as {
